...
class Make_Post {
   ...
   *
   * @param false|string $result Short-circuit return value. Either false or the value to replace the shortcode with.
   ...
   *
   */
  public function pre_do_shortcode_tag( $result, $tag, $attr, $m ) {
    if( POST_FROM_EMAIL_SLUG === $tag ) {
      return $this->excerpt;
    }
    return $result;
  }
  ...
  public function process() {
    ...
    try {
      ...
      if( array_key_exists( 'plain', $this->upload ) ) {
        $this->excerpt = $this->upload['plain'];
      } else {
        $this->excerpt = $this->getElementContents( $doc, '/html/body', '' );
        $this->excerpt = preg_replace( '/\s+/mu', ' ', $this->excerpt );
      }
      ...
      $post = array(
        ...
        'post_excerpt' => substr( $this->excerpt, 0, 160 ),
      ...
      /* Suppress any attempt to expand the shortcode from the not-yet-stored metadata. Yoast tries this. */
      add_filter( 'pre_do_shortcode_tag', array( $this, 'pre_do_shortcode_tag' ), 10, 4 );
      $id = wp_insert_post( $post, true, true );
      remove_filter( 'pre_do_shortcode_tag', array( $this, 'pre_do_shortcode_tag' ), 10 );
